...
export default function Hero({ onCategorySelect }: HeroProps) {
    // Original states for rotating words
    const words = ["directory", "blog", "job board", "helpdesk", "documentation"];
    const [index, setIndex] = useState(0);

    // --- New state for button hover effect ---
    ...
    }, [words.length]); // Added words.length dependency
                ...
                <div className="pt-12 pb-12">
                    ...
                    >
                        ...
                        <p className="text-xl text-slate-900 mb-8 font-lato max-w-2xl m-auto">
                            ...
                            {["Listings", "SEO", "Custom Domains", "Payments"].map((category, index) => (
                                <React.Fragment key={category}> {/* Use React.Fragment for key */}
                                    {index > 0 && ", "}
                                    <motion.b
                    ...
                    >
                        ...
                        <Link
                            ...
                            className="flex items-center justify-center py-3 px-6 bg-orange-600 text-white rounded-md font-medium hover:bg-orange-700 transition-colors w-full sm:w-auto relative" // Added relative
                            onMouseEnter={() => setIsHovered(true)}
                            onMouseLeave={() => setIsHovered(false)}
                        >
                            {/* Container for the flipping text - adjust height (e.g., h-5) as needed based on your font/line-height */}
                            <div className="relative overflow-hidden mr-2 h-5">
                                ...
                                {/* Hover Text - Change "Get Started Now" if needed */}
                                <div
                                    className="absolute top-0 left-0 w-full text-center transition-transform duration-300"
                                    style={{
                            ...
                            {/* Icon */}
                            <ArrowRight className="w-4 h-4 flex-shrink-0" /> {/* Added flex-shrink-0 */}
                        </Link>
