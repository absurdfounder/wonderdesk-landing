...
// Don't import TabImageHotspots here if using lazy loading

// Lazy load the component properly
...
// Extract SVGs to separate components with lightweight initial rendering
...
const NotionLogo = () => (
  <div className="w-10 h-10 mr-2 flex items-center justify-center rounded">
...
export default function Hero({ onCategorySelect }: HeroProps) {
  ...
  useEffect(() => {
    ...
    // Start rotating words immediately
  ...
  // Defer Cal widget loading to after critical content is displayed
  useEffect(() => {
    if (!isLoaded) return;

    let calApiLoaded = false;

    // Load Cal.com API with lower priority
    ...
    if (typeof window !== 'undefined' && 'requestIdleCallback' in window) {
      ...
      setTimeout(loadCalApi, 1000);
    }

    return () => {
      // Cleanup if needed
      if (calApiLoaded && typeof window !== 'undefined') {
        // Clean up Cal widget if there's an API for that
      }
    };
  }, [isLoaded]);
    ...
    <section className="relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="pt-12 pb-12">
          {/* Header section with immediate animation */}
          <motion.div
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-8 mb-8 items-center justify-center">
            ...
            >
              <div className="relative overflow-hidden mr-2 h-5">
                <div
                  className="transition-transform duration-150" // Even faster transition
                  style={{
                ...
                <div
                  className="absolute top-0 left-0 w-full text-center transition-transform duration-150" // Even faster transition
                  style={{
            ...
            {/* Book Demo Button - immediate rendering */}
            <button
              ...
              >
                <Suspense fallback={
                  <div className="h-64 w-full flex items-center justify-center">
                    <div className="w-full max-w-4xl mx-auto h-64 bg-gray-100 rounded-lg animate-pulse"></div>
                  </div>
                }>
                  <TabImageHotspots />
                </Suspense>
